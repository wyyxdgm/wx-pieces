...
Component({
  ...
  properties: {
    ...
    unload: { type: Boolean, value: false }, // 卸载时一次性执行，只执行一次
    delay: { type: Number, value: 1000 },
    title: { type: String, value: '相机开启中…' },
    unloadTitle: { type: String, value: '相机关闭中…' }
  },
  ...
  pageLifetimes: {
    show() {
      ...
      setTimeout(() => {
        if (this.data.showCamera) this.show();
        else this.hide();
      }, 500);
    },
    hide() {
  ...
  methods: {
    show() {
      ...
      this.setData({ show: true }, trigger);
    ...
    hide(keepLoading = false, unload = false) {
      this.setData({ show: false, loading: keepLoading }, () => {
    ...
    onCameraInit(e) {
      ...
      if(IS_DEVTOOLS) {
        this.auth_get_promise('scope.camera')
        .then(res => {
          this.handleOnCameraInit(e)
        })
        .catch(err => {
          if(!err?.authSetting['scope.camera']) {
            this.auth_promise('scope.camera')
            .then(res => {
              this.handleOnCameraInit(e)
            })
            .catch(msg => {
    ...
    handleOnCameraInit(e) {
      this.setData({ showTips: false,loading: false  });
      this.triggerEvent('initdone', e);
    },
    onCameraError(e) {
      conosole.error('onCameraError', e)
      this.setData({ showTips: true, loading: false, show: false });
    ...
    openAuthSetting() {
      ...
      this.auth_open_setting('scope.camera')
      .then(_ => {
